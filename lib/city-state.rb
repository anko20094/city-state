...
module CS
  # CS constants
  FILES_FOLDER = File.expand_path('db', __dir__)
  ...
  def self.install(country)
    ...
    states_replace = symbolize_keys(YAML.load_file(states_replace_fn))
    ...
    File.foreach(MAXMIND_DB_FN) do |line|
      ...
      # there's no correspondent in states-replace: we'll use the long name as code
      ...
      cities.merge!({ rec[STATE] => [] }) unless states.has_key?(rec[STATE])
      cities[rec[STATE]] << rec[CITY]

      # states list: {TX: "Texas", CA: "California"}
      unless states.has_key?(rec[STATE])
  ...
  def self.cities(state, country = nil)
    self.current_country = country if present?(country) # set as current_country
    ...
    if blank?(@cities[country])
      ...
      @cities[country] = symbolize_keys(YAML.load_file(cities_fn))
      ...
      unless lookup.nil?
        lookup.each do |state, new_values|
          new_values.each do |old_value, new_value|
            if new_value.nil? || blank?(new_value)
  ...
  def self.get_cities_lookup(country)
    ...
    if @cities_lookup.nil?
      ...
      return nil unless File.exist?(@cities_lookup_fn)

      @cities_lookup = symbolize_keys(YAML.load_file(@cities_lookup_fn)) # force countries to be symbols
      @cities_lookup.each { |key, value| @cities_lookup[key] = symbolize_keys(value) } # force states to be symbols
    end
  ...
  def self.get_states_lookup(country)
    ...
    if @states_lookup.nil?
      ...
      return nil unless File.exist?(@states_lookup_fn)

      @states_lookup = symbolize_keys(YAML.load_file(@states_lookup_fn)) # force countries to be symbols
      @states_lookup.each { |key, value| @states_lookup[key] = symbolize_keys(value) } # force states to be symbols
    end

    return nil unless @states_lookup.key?(country)
  ...
  def self.get_countries_lookup
    ...
    if @countries_lookup.nil?
      ...
      return nil unless File.exist?(@countries_lookup_fn)

      @countries_lookup = symbolize_keys(YAML.load_file(@countries_lookup_fn)) # force countries to be symbols
    end
  ...
  def self.states(country)
    ...
    if blank?(@states[country])
      ...
      @states[country] = symbolize_keys(YAML.load_file(states_fn))
